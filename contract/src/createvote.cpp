    ...
    const std::vector<answer> &answers)
{
    ...
    // eosio::check(poll->has_opened(), "poll has not opened yet");

    // // check if poll is closed
    // eosio::check(!poll->is_closed(), "poll is closed");
    ...
    //     eosio::check(prefilled_option_index >= -1, "invalid answer: option index cannot be less than -1");
    ...
    //     {
    //         eosio::check(voter_is_on_list, "voter is not on the allow list");
    //     }
    //     else
    //     {
    //         eosio::check(!voter_is_on_list, "voter is on the deny list");
    //     }
    // }
